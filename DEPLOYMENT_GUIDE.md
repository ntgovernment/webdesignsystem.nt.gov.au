...
- **Confirm Card toggles are enabled** - Verify `showImage` and/or `showIcon` are enabled on the Card instance
...
- **Inspect the rendered client image** - In Card `1.0.15+`, `showImage` triggers a cached `getGeneral` request for the Related Asset and selects the smallest original or variety covering the measured media container at the device pixel ratio
